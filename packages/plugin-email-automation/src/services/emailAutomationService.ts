...
export class EmailAutomationService extends Service {
    ...
    async initialize(runtime: IAgentRuntime): Promise<void> {
        ...
        elizaLogger.debug("🔄 Initializing Email Automation Service...");
        ...
        try {
            ...
            elizaLogger.debug("📧 Email service ready to process messages");
        } catch (error) {
    ...
    async evaluateMessage(memory: Memory): Promise<boolean> {
        ...
        try {
            ...
            elizaLogger.debug("🔍 Evaluating accumulated conversation for email automation:", {
            ...
            if (shouldEmail) {
                elizaLogger.debug("✨ Accumulated context triggered email automation, preparing to send...");
                await this.handleEmailTrigger(context);
                elizaLogger.success("✅ Email processed and sent successfully");
                return true;
            }

            elizaLogger.debug("⏭️ Current context does not warrant email automation");
            return false;
    ...
    private async shouldSendEmail(context: EmailContext): Promise<boolean> {
        elizaLogger.debug("🤔 Evaluating if message should trigger email...");
        ...
        elizaLogger.debug("📝 Final composed prompt:", {
        ...
        elizaLogger.debug(`📊 Email decision: ${shouldEmail ? "✅ Should send" : "❌ Should not send"}`, {
    ...
    private async handleEmailTrigger(context: EmailContext) {
        try {
            ...
            elizaLogger.debug("📋 Email content prepared:", {
            ...
            elizaLogger.debug("📤 Composing email...", {
                to: emailOptions.to,
