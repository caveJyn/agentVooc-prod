...
export const generateEmailAction: Action = {
    ...
    async validate(_runtime: IAgentRuntime, message: Memory): Promise<boolean> {
        ...
        elizaLogger.debug('Generate validation:', {
            isValid,
    ...
    async handler(
        ...
        try {
            ...
            elizaLogger.debug('Generate handler started', {
                messageId: message.id,
                hasState: !!state,
            ...
            elizaLogger.debug('Generating email with prompt:', { prompt });

            const generatedEmail = await emailService.generateEmail(prompt);
            elizaLogger.debug('Email generated:', {
            ...
            if (state) {
                ...
                elizaLogger.debug('State updated and persisted', {
...
${generatedEmail.blocks.map(block => {
    switch(block.type) {
        ...
        default:
            ...
            elizaLogger.debug('Email generation completed successfully');
