...
export class TwitterPostClient {
  ...
  constructor(client: ClientBase, runtime: IAgentRuntime, state: any) {
    ...
    logger.log(
      `- Post Interval: ${this.state?.TWITTER_POST_INTERVAL_MIN || this.runtime.getSetting('TWITTER_POST_INTERVAL_MIN') || 5}-${this.state?.TWITTER_POST_INTERVAL_MAX || this.runtime.getSetting('TWITTER_POST_INTERVAL_MAX') || 180} minutes`
    );
    logger.log(
      `- Post Immediately: ${
