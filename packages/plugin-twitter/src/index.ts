...
async createClient(
    ...
    // Remove the setSetting call; handle null TWITTER_2FA_SECRET in state or config
    try {
  ...
  async initialize(runtime: IAgentRuntime): Promise<void> {
    const twitterConfig: Partial<TwitterConfig> = {
        TWITTER_USERNAME:
            ...
            runtime.character.settings?.secrets?.TWITTER_USERNAME,
        TWITTER_PASSWORD:
            (runtime.getSetting('TWITTER_PASSWORD') as string) ||
            runtime.character.settings?.secrets?.TWITTER_PASSWORD,
        TWITTER_EMAIL:
            (runtime.getSetting('TWITTER_EMAIL') as string) ||
            runtime.character.settings?.secrets?.TWITTER_EMAIL,
        TWITTER_2FA_SECRET:
            (runtime.getSetting('TWITTER_2FA_SECRET') as string) ||
            runtime.character.settings?.secrets?.TWITTER_2FA_SECRET,
    };
    ...
    if (
        ...
        logger.info('Initializing Twitter client from character settings');
        await this.createClient(runtime, runtime.agentId, config);
    } else {
        logger.warn('Twitter credentials incomplete; client not initialized');
    }
}
